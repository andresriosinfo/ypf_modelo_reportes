...
Este módulo implementa un protocolo completo para:
1. Identificar variables adecuadas para detección de anomalías
2. Limpiar y preprocesar los datos
3. Validar la calidad de las variables seleccionadas
4. Exportar el dataset limpio y listo para ML
"""
...
class AnomalyDetectionVariableProtocol:
    ...
    def process_file(self, 
        ...
        """
        Procesa un archivo Excel completo: selecciona variables y limpia datos
    ...
    def process_multiple_files(self,
                              ...
                              datetime_col: str = 'DATETIME',
                              output_dir: str = 'output') -> Dict[str, Tuple[pd.DataFrame, pd.DataFrame]]:
        """
        Procesa múltiples archivos y encuentra variables comunes
        
        Parámetros:
        -----------
        file_patterns : List[str]
            Lista de patrones/rutas de archivos
        datetime_col : str
            Nombre de la columna de fecha/hora
        output_dir : str
            Directorio para guardar outputs
        ...
        Retorna:
        --------
        Dict : Diccionario con resultados por archivo
        """
        results = {}
        all_selected_vars = []
        ...
        for file_path in file_patterns:
            if Path(file_path).exists():
                cleaned, report = self.process_file(file_path, datetime_col, 
                                                   save_report=True, output_dir=output_dir)
                results[file_path] = (cleaned, report)
                all_selected_vars.append(set(self.selected_variables))
        
        # Encontrar variables comunes a todos los archivos
        if all_selected_vars:
            common_vars = set.intersection(*all_selected_vars)
            print(f"\n{'='*80}")
            print(f"VARIABLES COMUNES EN TODOS LOS ARCHIVOS: {len(common_vars)}")
            print(f"{'='*80}")
            if common_vars:
                print("\nVariables recomendadas para modelo unificado:")
                for var in sorted(list(common_vars))[:20]:
                    print(f"  - {var}")
                if len(common_vars) > 20:
                    print(f"  ... y {len(common_vars) - 20} más")
...
def main():
    ...
    # Si se pasan archivos como argumentos, procesarlos
    if len(sys.argv) > 1:
        files = sys.argv[1:]
    else:
        # Archivos de ejemplo
    ...
    print(f"Variables seleccionadas por archivo: {len(protocol.selected_variables)}")
